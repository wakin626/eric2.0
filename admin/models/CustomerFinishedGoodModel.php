...
class CustomerFinishedGoodModel extends BaseModel {
    protected $table = 'customer_finished_goods';

    public function getAll($activeOnly = true) {
        $sql = "SELECT fg.*, c.customer_name FROM {$this->table} fg
                LEFT JOIN customers c ON fg.customer_id = c.customer_id AND c.`remove` = 0
                WHERE fg.`remove` = 0";
        if ($activeOnly) {
            $sql .= " AND fg.status = 1";
        }
        $sql .= " ORDER BY fg.item_id DESC";
    ...
    public function create($data) {
        ...
        $sql = "INSERT INTO {$this->table} (item_code, item_description, customer_id, item_uom, uom_conversion, item_amount, status) 
                VALUES (:item_code, :item_description, :customer_id, :item_uom, :uom_conversion, :item_amount, :status)";
    ...
    public function toggleStatus($id) {
        $sql = "UPDATE {$this->table} SET status = IF(status = 1, 0, 1) WHERE item_id = :id AND `remove` = 0";
        $stmt = self::getConnection()->prepare($sql);
        return $stmt->execute(['id' => $id]);
    }

    public function updateConversion($id, $conversion) {
        $sql = "UPDATE {$this->table} SET uom_conversion = :conv WHERE item_id = :id AND `remove` = 0";
        $stmt = self::getConnection()->prepare($sql);
        return $stmt->execute(['conv' => $conversion, 'id' => $id]);
    }
    ...
    public function getAllFiltered($filters = [], $activeOnly = false) {
        ...
        if (!empty($filters['search'])) {
            $like = '%' . $filters['search'] . '%';
            $sql .= " AND (fg.item_code LIKE :search1 
                       OR fg.item_description LIKE :search2
                       OR c.customer_name LIKE :search3
                       OR fg.item_uom LIKE :search4)";
            $params['search1'] = $like;
            $params['search2'] = $like;
            $params['search3'] = $like;
            $params['search4'] = $like;
        ...
        if (!empty($filters['customer_id'])) {
            $sql .= " AND fg.customer_id = :filter_customer_id";
