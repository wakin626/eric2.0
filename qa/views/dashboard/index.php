<h4><i class="bi bi-clipboard-check me-2"></i>QA Dashboard</h4>
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
    <div class="d-flex gap-2 flex-wrap">
        <form method="GET" id="qaFilterForm" class="d-flex gap-2 flex-wrap">
        ...
        <a href="?controller=qa" class="btn btn-sm btn-outline-secondary"><i class="bi bi-x-circle me-1"></i>Clear</a>
    </div>
        ...
        <table class="table table-hover mb-0">
            ...
            <tbody id="historyTableBody">
                ...
                <tr>
                    ...
                    <td>
                        <?php if (!empty($h['qa_remark'])): ?>
                            <button class="btn btn-sm btn-outline-warning" onclick="openEditRemarkModal(<?= $h['history_id'] ?>, '<?= htmlspecialchars(addslashes($h['lot_number'] ?? ''), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($h['item_description'] ?? ''), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($h['qa_remark'] ?? ''), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($h['qa_inspector_name'] ?? ''), ENT_QUOTES) ?>')">
...
<div class="modal fade" id="remarkModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="?controller=qa&action=updateRemark">
                <div class="modal-header">
                    <h5 class="modal-title" id="remarkModalTitle"><i class="bi bi-clipboard-check me-2"></i>QA Inspection Remark</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="qa_inspector_name" id="remarkInspectorName" class="form-control" placeholder="Enter your name" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">QA Remark <span class="text-danger">*</span></label>
                        <textarea name="qa_remark" id="remarkText" class="form-control" rows="3" placeholder="Enter inspection remark..." required></textarea>
                    </div>
                </div>
...
<nav>
    <ul class="pagination justify-content-center mt-4">
        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
            <a class="page-link" href="?controller=qa&page=<?= $page - 1 ?>&search=<?= urlencode($search ?? '') ?>">&laquo; Prev</a>
        </li>
        <?php foreach ($pages as $p): ?>
            ...
            <li class="page-item <?= $p == $page ? 'active' : '' ?>">
                <a class="page-link" href="?controller=qa&page=<?= $p ?>&search=<?= urlencode($search ?? '') ?>"><?= $p ?></a>
            </li>
            <?php endif; ?>
        <?php endforeach; ?>
        <li class="page-item <?= $page >= $totalPages ? 'disabled' : '' ?>">
            <a class="page-link" href="?controller=qa&page=<?= $page + 1 ?>&search=<?= urlencode($search ?? '') ?>">Next &raquo;</a>
        </li>
    </ul>
</nav>
<?php endif; ?>
...
function openAddRemarkModal(historyId, lotNumber, itemDescription) {
    ...
    document.getElementById('remarkModalTitle').innerHTML = '<i class="bi bi-clipboard-check me-2"></i>QA Inspection Remark';
...
function openEditRemarkModal(historyId, lotNumber, itemDescription, existingRemark, inspectorName) {
    ...
    document.getElementById('remarkModalTitle').innerHTML = '<i class="bi bi-pencil me-2"></i>Edit QA Remark';
