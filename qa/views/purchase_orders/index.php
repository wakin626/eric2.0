...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
    <div class="d-flex gap-2 flex-wrap">
        ...
        <select class="form-select form-select-sm filter-select" style="width:180px" onchange="location.href='?controller=qa&action=purchaseOrders&delivery_status=' + encodeURIComponent(this.value)">
        ...
        <a href="?controller=qa&action=purchaseOrders" class="btn btn-sm btn-outline-secondary"><i class="bi bi-x-circle me-1"></i>Clear</a>
    </div>
...
<th class="text-center">Actions</th>
            ...
            <tbody id="poTableBody">
                ...
                <tr>
                    ...
                    <?= htmlspecialchars($po['customer_po_number']) ?>
                    </strong></td>
...
<td>
    <?php if (!empty($items)): ?>
        ...
        ?>
            <?= $idx > 0 ? '<hr class="my-1 border-secondary">' : '' ?>
                <div class="d-flex align-items-center gap-2" style="min-height: 20px;">
...
<?php $paginationParams = http_build_query(array_filter(['controller'=>'qa','action'=>'purchaseOrders','search'=>$search??'','filter_customer'=>$filterCustomer??'','filter_item'=>$filterItem??'','filter_date'=>$filterDate??''])); ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadLots(poiId) {
        ...
        foot.style.display = 'none';
        fetch('?controller=warehouse&action=getLotsByPOItem&poi_id=' + poiId + '&_t=' + Date.now())
