<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <a href="?controller=warehouse&action=deliveries" class="btn btn-outline-secondary">
    ...
    <div class="d-flex gap-2 flex-wrap">
        ...
        <a href="?controller=warehouse&action=viewBackloads" class="btn btn-sm btn-outline-secondary"><i class="bi bi-x-circle me-1"></i>Clear</a>
    </div>
        ...
        <nav>
            <ul class="pagination pagination-sm mb-0">
                <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="?controller=warehouse&action=viewBackloads&page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&filter_customer=<?= urlencode($filterCustomer) ?>">Previous</a>
                </li>
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                        <a class="page-link" href="?controller=warehouse&action=viewBackloads&page=<?= $i ?>&search=<?= urlencode($search) ?>&filter_customer=<?= urlencode($filterCustomer) ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>
                <li class="page-item <?= $page >= $totalPages ? 'disabled' : '' ?>">
                    <a class="page-link" href="?controller=warehouse&action=viewBackloads&page=<?= $page + 1 ?>&search=<?= urlencode($search) ?>&filter_customer=<?= urlencode($filterCustomer) ?>">Next</a>
                </li>
            </ul>
        </nav>
    </div>
