...
<form id="moEntryForm" class="mo-card p-4" method="POST" action="?controller=mo&action=store">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateLineDetails() {
        ...
        fetch('?controller=warehouse&action=getItemBomInfo&item_id=' + encodeURIComponent(selectedOption.value))
            .then(response => response.json())
            .then(data => {
    ...
    document.getElementById('moEntryForm').addEventListener('submit', function () {
        syncLineItemsPayload();
    });
